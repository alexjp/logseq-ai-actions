...
### Per-block diff across subtree (2026-06-06)

Two new `ActionScope` values (`subtree-per-block`, `subtree-batched`) that fan an action out across a node and its descendants with a multi-block diff panel. Both restricted to `outputMode: "diff-panel"` by a Zod `.superRefine`. Children are never replaced or restructured — each block's text is updated in place via `logseq.Editor.updateBlock`.

- [x] **Planning** — locked two scopes (not one scope + a `strategy` field), per-block sequential (not parallel), auto-fall-back to per-block on batched alignment failure, hard cap 50 / soft warn 20, edit-implies-accept, empty-response auto-reject, no seed action (users author their own). All decisions from `AskUserQuestion` rounds.
- [x] **`src/subtree-walk.ts`** (new, pure, no SDK) — `walkSubtree(root, options)` returns `{ status: "ok", nodes, totalSeen, warned }` or `{ status: "too-large", totalSeen, cap }`; DFS pre-order; depth tags; empty-block filter; defaults `softWarnAt=20` / `hardCapAt=50`; explicit stack; throws on `softWarnAt < 1` or `hardCapAt < softWarnAt`.
- [x] **`src/parse-outline.ts`** — added `flattenOutlineTree(nodes)` returning `{ text, depth, index }[]` (parent-first DFS, explicit stack, reverse-push for left-to-right). Used by `alignBatchedResponse`.
- [x] **`src/types.ts` + `src/action.ts`** — `ActionScope` extended with `subtree-per-block` and `subtree-batched`; `ActionSchema.superRefine` pins both to `outputMode: "diff-panel"` and rejects other modes with a clear `path: ["outputMode"]` issue.
- [x] **`src/adapter/llm-runner.ts`** (new) — extracted `buildProviderRequest`, `formatProviderError`, `recordDebugEntry`, `closeBusyToast`, `performLLM` from `run-action.ts`; takes `llmInput: string` directly so per-block and batched runners share one boundary.
- [x] **`src/adapter/run-action.ts`** — refactored to use the extracted helpers; new scope branches sit between model-check and `resolveInput`.
- [x] **`src/adapter/run-per-block-action.ts`** (new) — resolves parent uuid (explicit or current), loads subtree via `logseq.Editor.getBlock(uuid, { includeChildren: true })`, walks it, surfaces `too-large` / `warned` / `empty-subtree` toasts, opens `showMultiBlockDiff` with a `textByUuid` lookup closure, applies accepted blocks sequentially, final success/cancel toasts.
- [x] **`src/adapter/run-batched-action.ts`** (new) — same skeleton; `flattenSubtree` strips `uuid` via a `StrippedNode` type to satisfy `exactOptionalPropertyTypes`; one non-streaming `performLLM` call; `alignBatchedResponse(walked, output)` extracted as a pure export returning `aligned` (proposals map) or `misaligned` (observed/expected counts); on `misaligned` shows toast + calls `runPerBlockAction` recursively; on `aligned` opens the panel with `runOneBlock` emitting pre-computed text as a single chunk.
- [x] **`src/ui/MultiBlockDiffPanel.tsx`** (new, ~430 lines) — stacked cards; per-card `CardState { uuid, depth, original, isParent, hasChildren, proposed, editedText, status, errorMessage }`; `status: "pending" | "streaming" | "empty" | "accepted" | "rejected" | "edited"`; sequential streaming via `useEffect` on `streamingIndex` with `settledRef` + `streamGen` for cancellation and stale-chunk drops; per-card ✓ (disabled on streaming/empty), ✗, ✎ (textarea edit with Save/Cancel — Save marks `edited` and accepts); global footer with `Reject remaining` + `Apply N` (disabled while streaming); Esc/⌘↵ shortcuts; inline empty-response note; `BlockCard` sub-component renders unified diff (same/removed/added inline) for finished cards, streaming/empty placeholders otherwise.
- [x] **`src/ui/show-multi-block-diff.ts`** (new) — `showMultiBlockDiff(options)` wraps `mountPanel`; resolves with `ReadonlyArray<{ uuid, text }>` on Apply or `null` on Cancel; mirrors `showDiffPanel`.
- [x] **Manage Actions UI** — `SCOPE_HINTS` extended (`subtree-per-block`, `subtree-batched`); `PillRadio` scope options extended; Import JSON placeholder shows both `subtree + append-children` AND `subtree-per-block + diff-panel` examples; `categorizeAction` (`picker-categories.ts`) routes the new scopes into the Transform bucket **before** the id-prefix check so a user action `grammar-subtree` lands in Transform, not Fix.
- [x] **CSS in `index.html`** — new `.multi-root` (900px), `.multi-streaming-bar`, `.multi-body` (scrollable), `.multi-card*` (depth-indented, accepted/rejected tint), `.multi-icon-btn*` (✓/✗/✎), `.multi-status*` (streaming/empty/accepted/rejected/edited with `color-mix(in oklab, var(--success|--danger|--accent) …)`), `.multi-card-body` (2-column grid), `.multi-empty-note`, `.multi-counts`, `@keyframes multi-pulse` for streaming pill. Reuses `.diff-modal` / `.diff-header` / `.diff-footer` / `.diff-btn` / `.diff-pre` / `.diff-edit` / `.diff-added` / `.diff-removed`.
- [x] **Tests**
  - `src/subtree-walk.test.ts` (new) — 14 cases: single-block, flat children, recursive depth + `hasChildren`, empty/whitespace filter, `title ?? content` precedence, `children: []` treated as no children, warn threshold (default + custom), too-large at cap, off-by-one at cap, post-filter cap (50 non-empty among 100 children), threshold-validation throws.
  - `src/parse-outline.test.ts` (extended) — 6 new `flattenOutlineTree` cases (empty, single, multi-block DFS pre-order, multi-depth left-to-right with contiguous indices, count parity with `countOutlineNodes`, all-leaves).
  - `src/action.test.ts` (extended) — 2 new `it.each` blocks for `subtree-per-block` / `subtree-batched` rejecting each non-`diff-panel` outputMode with a `path: ["outputMode"]` issue; 3 accept-cases.
  - `src/adapter/run-batched-action.test.ts` (new) — 10 `alignBatchedResponse` cases: single-node, multi-block index-aligned, depth-flattened-but-aligned, observed>expected, observed<expected, empty LLM output, both-empty, parseOutline preamble+code-fence tolerance, 50-block cap boundary, 51-vs-50 boundary.
  - `src/ui/picker-categories.test.ts` (1-line fix) — existing `a()` helper now emits `scope: "block"` to satisfy the tightened `categorizeAction` signature.
- [x] **Final test pass** — 25 test files, **397 tests passing**, 94.47% statements / 90.32% branches / 96.42% functions / 97.51% lines (all well above 80%). `pnpm typecheck` clean, `pnpm lint` clean.
- [x] **User verify** ("That was an amazing work! So impressed! Congrats!") — `pnpm dev` + reload; per-block and batched paths both worked; per-card Accept/Reject/Edit all behave; alignment-fallback path implicitly trusted.
- [x] **Docs** — `REQUIREMENTS.md §4` scope table extended; new `§18` added (panel contract, edit-implies-accept, empty-response handling, size policy, batched alignment rule, authoring surface, why no seed action, out-of-scope). `README.md` Features list extended; new "Per-block diff across subtree" subsection with `userActionsJson` example.
- [x] **Changeset** — `.changeset/subtree-per-block.md` (minor bump 1.2.0 → 1.3.0) describing the feature; `pnpm changeset version` to apply. Final state: `package.json` at `1.3.0`, `CHANGELOG.md` has the 1.3.0 entry.
- [x] **Final gate** — `pnpm typecheck && pnpm lint && pnpm test` clean. 25 test files / 397 tests passing; 94.47% / 90.32% / 96.42% / 97.51% coverage. (Biome auto-fixed 3 files on the way through: a `parse-outline.test.ts` long-line collapse + 2 import sorts.)

### Retry button (2026-06-06, 1.3.1)

User request: "in the diff editor, were it has the buttons 'Reject', 'Edit', 'Accept', add a new button 'Retry' that makes a new call to the LLM (exactly like the original) in cases that the first result of the LLM is not great." Followed by the user choosing to also add the affordance to the multi-block panel; on the batched path Retry re-invokes the LLM for that one block with the original text (per-block fallback for the retried card, other cards keep their cached batched proposals). Dirty-edit guard: reuse the existing `ConfirmOverlay` from the action-bar switch path. Version: 1.3.1 patch.

- [x] **`src/ui/DiffPanel.tsx`** — new `pendingRetry` state (mirrors `pendingSwitchActionId`); `handleRetry` bails on `busy`; if `isEditing && editedText !== proposed` sets `pendingRetry` (overlay path), else `void startStream(currentActionId)`. New footer button between Edit-or-Cancel-edit and Accept: `<button class="diff-btn" disabled={busy} onClick={handleRetry} title="Re-run the LLM with the same input">Retry</button>`. The button is NOT gated on `proposed.length === 0` — a failed first attempt (`errorMessage` set, `proposed` empty) must be retryable. The existing `ConfirmOverlay` block extended: also renders when `pendingRetry`; `onCancel` clears both `pendingSwitchActionId` and `pendingRetry`; `onConfirm` branches on `pendingRetry` (re-streams current) vs `pendingSwitchActionId` (existing path). Copy: title stays "Discard your edits?"; message + confirmLabel are retry-specific ("Re-running will replace your edited text with a fresh proposal." / "Discard and retry"); switch copy unchanged.
- [x] **`src/ui/MultiBlockDiffPanel.tsx`** — new optional `retryBlock?: RunOneBlock` prop on `MultiBlockDiffPanelProps` (hidden when omitted). New `handleCardRetry(i)` callback in the parent: snaps card `i` to `status: "streaming"`, clears `proposed` / `editedText` / `errorMessage`; calls `retryBlock(uuid, onChunk)`; reuses the existing `streamGen` + `settledRef` guards so a Cancel-while-Retry discards stragglers and a mid-stream card can't be retried again. On resolve, applies the same auto-reject-empty rule as the main sequential useEffect (`status: "empty"` if `finalText.trim()` empty, else `status: "pending"` with `proposed` set); on error, `status: "empty"` + `errorMessage`. New `onRetry?: () => void` prop on `BlockCardProps`; the `BlockCard` renders a per-card `↻` glyph after the `✎` Edit button, disabled while the card is `streaming`. Parent passes `onRetry={retryBlock ? () => handleCardRetry(i) : undefined}` via conditional spread (required for `exactOptionalPropertyTypes: true`).
- [x] **`src/ui/show-multi-block-diff.ts`** — `ShowMultiBlockDiffOptions` gains `readonly retryBlock?: RunOneBlock`; forwarded to the panel via conditional spread.
- [x] **`src/adapter/run-per-block-action.ts`** — wires `retryBlock` to the same closure body as `runOneBlock` (3-line LLM call, duplicated rather than aliased for clarity).
- [x] **`src/adapter/run-batched-action.ts`** — builds `textByUuid` map alongside `proposedByUuid` (one-line loop); wires `retryBlock` to a per-block `performLLM` call with the original text (batched proposal for that card is abandoned, other cards keep their cached batched proposals).
- [x] **CSS in `index.html`** — added `.multi-icon-retry` rule (color: `var(--muted)`); the rest of the `.multi-icon-btn*` styling is shared.
- [x] **Docs** — `REQUIREMENTS.md` status line extended; §18 per-card-button line updated to list `↻ Retry`; new "Per-block Retry" subsection in §18 covering single-block + multi-block affordance, dirty-edit overlay reuse, and the batched-path per-card-LLM-reinvoke semantics. `tasks.md` updated.
- [x] **No new tests** — `DiffPanel.test.ts` only covers pure exports; the panel components and SDK-glue runners have no DOM test infra. The retry-block path is exercised end-to-end in Logseq.
- [x] **Changeset** — `.changeset/retry-button.md` (patch bump 1.3.0 → 1.3.1) describing the affordance on both panels + batched-path semantics + Edit-mode overlay behavior. `pnpm changeset version` applied; `package.json` at `1.3.1`; `CHANGELOG.md` has the 1.3.1 entry.
- [x] **Final gate** — `pnpm typecheck && pnpm lint && pnpm test` clean. 25 files / 397 tests passing; 94.47% / 90.32% / 96.42% / 97.51% coverage (unchanged).

### Bulk accept (2026-06-06, 1.3.2)

User request: "In the development of multiple block diff panel, there is an option missing: Accept all changed. Imagine I have some 10 nodes, and 7 of them have changes and are correct. I could just click in a button and accept all those changed, instead of having to click 7 times the accept button." Followed by the user choosing: label `Accept all changed (N)`, version 1.3.2 patch (on top of the uncommitted 1.3.1), skip the `Reject all changed` mirror, and bind `⌘⇧A` as the keyboard shortcut.

- [x] **`src/ui/MultiBlockDiffPanel.tsx`** — new `handleAcceptAllChanged` callback (filters cards, flips `pending && proposed.length > 0 && proposed.trim() !== original.trim()` to `status: "accepted"`; leaves `accepted`/`rejected`/`edited`/`streaming`/`empty` and unchanged `pending` alone). New `changedPendingCount` `useMemo` with the same predicate (drives the button label and disabled state). New footer button between `Reject remaining` and `Apply N`: `<button class="diff-btn" onClick={handleAcceptAllChanged} disabled={counts.streaming > 0 || changedPendingCount === 0} title={...}>Accept all changed{changedPendingCount > 0 ? ` (${changedPendingCount})` : ""}</button>`. Global keydown handler extended: `⌘⇧A` (meta + shift + A/a) calls `handleAcceptAllChanged()`. Hint line updated: `<kbd>Esc</kbd> cancel · <kbd>⌘⇧A</kbd> accept all · <kbd>⌘↵</kbd> apply`.
- [x] **No CSS** — reuses `.diff-btn` styling; not a primary action, no `.diff-btn-primary` variant needed. Muted/secondary button fits between the destructive `Reject remaining` and the primary `Apply N`.
- [x] **No new tests** — `DiffPanel.test.ts` only covers pure exports; the panel components have no DOM test infra. The bulk-accept path is exercised end-to-end in Logseq.
- [x] **Docs** — `REQUIREMENTS.md` status line extended with the new affordance; §18 per-card-buttons paragraph updated (footer now lists `Accept all changed (N)` between `Reject remaining` and `Apply N`; keyboard hint extended with `⌘⇧A`); new "Bulk accept (multi-block panel only)" subsection in §18 covering predicate, disabled-when rules, the "marks-don't-commit" semantics, the absence of a `Reject all changed` mirror, and the unchanged-`pending`-cards-filtered-at-apply-time behavior. `tasks.md` updated.
- [x] **Changeset** — `.changeset/accept-all-changed.md` (patch bump 1.3.1 → 1.3.2) describing the bulk-accept affordance + `⌘⇧A` shortcut + the additive nature of the action (leaves terminal-status cards alone, unchanged `pending` cards stay `pending` and filter out at apply time). `pnpm changeset version` applied; `package.json` at `1.3.2`; `CHANGELOG.md` has the 1.3.2 entry.
- [ ] **Final gate** — `pnpm typecheck && pnpm lint && pnpm test`. (In progress.)

### Streaming fetch bypass (2026-06-06, 1.3.3)

User report: in Multi Diff Edit, every block streams with a noisy `console.warn` from `host-scope.ts:64` — `logseq-ai-actions: logseq.Request failed, falling back to fetch Error: ... SyntaxError: Unexpected token 'd', "data: {"ch"... is not valid JSON`. Root cause: the provider's streaming path was being routed through `logseq.Request._request`, which reads the full response body and `JSON.parse`s it; SSE responses start with `data: {` and crash. The fallback to `globalThis.fetch` always worked — the warning was the only visible symptom. Fix landed as 1.3.3 patch on top of uncommitted 1.3.2.

- [x] **`src/provider.ts`** — extended `ProviderOptions` with `streamFetchImpl?: typeof globalThis.fetch`; `createOpenAIProvider` resolves `streamFetchFn = options.streamFetchImpl ?? globalThis.fetch` and routes `openAIStream` through it (the non-streaming `openAIComplete` / `openAIVisionComplete` still use `fetchImpl`). Both fetchers are bound to `globalThis` at construction time so the call site in `postChat` — which invokes the fetcher as a property of a plain `PostChatOptions` object — preserves the Window receiver the native `fetch` requires; without this, streaming would throw `TypeError: Illegal invocation` in the plugin iframe. JSDoc on `ProviderOptions` explains the asymmetry: the SDK shim is structurally incompatible with SSE (JSON-parses the body) and must NOT be set as the streaming fetcher. JSDoc on `LLMProvider.stream` cross-references `ProviderOptions`.
- [x] **`src/index.ts`** — `createOpenAIProvider({ fetchImpl: logseqFetch })` unchanged; `streamFetchImpl` deliberately omitted so streaming always falls through to `globalThis.fetch`. Added a 2-line comment above the call documenting the reason.
- [x] **`src/adapter/host-scope.ts`** — untouched. The shim is correct for non-streaming JSON HTTP and the `console.warn` still fires for genuinely-broken non-streaming requests (e.g., 401 reflected through the SDK), which is the right behaviour. Only the streaming path needs to skip the shim.
- [x] **Tests** — two new cases in `src/provider.test.ts`'s `describe("stream", ...)` block: (1) when only `fetchImpl` is injected, `openAIStream` uses `globalThis.fetch` and ignores the injected one; (2) when `streamFetchImpl` is injected, `openAIStream` uses it and `globalThis.fetch` is NOT called; (3) a strict-mock regression test for the Illegal-invocation bug — uses a `vi.fn(function (this: unknown) { if (this !== globalThis) throw …; … })` mock as `globalThis.fetch` to enforce the receiver contract that the default lenient `vi.fn()` does not. The existing `fetchImpl` test at the non-streaming section is untouched.

  **Bugfix (amended to 1.3.3)**: the multi-block diff panel's `⌘⇧A` bulk-accept shortcut did not fire on Linux/Windows. The matcher in `src/ui/MultiBlockDiffPanel.tsx` checked `e.metaKey && e.shiftKey` (Mac-only) so `Ctrl+Shift+A` was a no-op. Fixed by switching the modifier check to `(e.metaKey || e.ctrlKey)` — same convention as the Enter handler four lines below and every other panel in the codebase — and the key check to `e.code === "KeyA"` for layout-independence (Dvorak/Cyrillic/AZERTY). No new test added: the panel has no test infrastructure (jsdom / @testing-library/preact are not installed; v1.3.0 decision "no new tests for panel"). Hint text `⌘⇧A` unchanged — Mac glyph with implicit `Ctrl` on Linux/Windows, the established cross-platform convention.
- [x] **Docs** — `REQUIREMENTS.md` status line extended with a one-sentence note about the SSE-incompatible shim. No §18 change (no user-facing behaviour change). `tasks.md` updated.
- [x] **Changeset** — `.changeset/streaming-fetch-bypass.md` (patch bump 1.3.2 → 1.3.3) describing the streaming bypass + the new `streamFetchImpl` option + the per-block warning removal. `pnpm changeset version` applied; `package.json` at `1.3.3`; `CHANGELOG.md` has the 1.3.3 entry.
- [ ] **Final gate** — `pnpm typecheck && pnpm lint && pnpm test`. (In progress.)
